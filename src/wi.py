...
class Channel:
    ...
    def get_defects_dence(self):
        ...
        # преобразование времени в фреймы
        step_seg = int(self.Parent.SampleRate * time / 512 + 1)

        # транспонирование, для работы со временем
        xht = xh.T

        # сегментация спектограммы
        segments = [xht[i:i + step_seg] for i in range(0, len(xht), int(step_seg))]

        # посегментный анализ
        for segt in segments:

            # транспонирование для работы с частотами
            xh = segt.T
